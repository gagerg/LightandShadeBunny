...
window.onload = function init()
{
    ...
    //This is where to implement the light that follows the mouse
...
function render()
{
    ...
    //Mouse Light
    setUniform3f(program,"lightMouseDir", lx, ly, .5);
    setUniform3f(program,"lightMouseColor", .5, .8, .1);

    //Rotating Light
    setUniform3f(program,"rotLightDir", Math.cos(angle), Math.sin(angle), -.5);
    setUniform3f(program,"rotLightColor", 0.9, .3, .9);

    //These won't have to change. The overall presense
    ...
    //uniforms set to that of shaders.js
